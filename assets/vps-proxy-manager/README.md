...
Codex is used to install, configure, test, upgrade, and diagnose this control-plane project. Telegram runtime operations are deterministic backend tasks, not arbitrary shell generated by Codex from Telegram messages.
...
- `切回本地出口`: disables and stops sing-box, so reboot keeps using the VPS local outbound path.
- `启用代理`: enables and starts sing-box with the current managed config.
