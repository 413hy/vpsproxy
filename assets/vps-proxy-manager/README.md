...
应用新节点时目标先保存备份和服务状态，再启动自动回滚 timer。原 SSH 命令返回后，目标通过公开的 systemd activation timer 延迟启动 TUN，控制端使用全新 SSH 连接确认服务稳定和公网出口。配置检查、服务启动、SSH 重连或公网探测任一失败时，不更新数据库当前节点，rollback timer 会恢复切换前状态。
