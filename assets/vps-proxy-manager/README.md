...
默认 Codex 配置为当前 ChatGPT 账号已验证可用的 `gpt-5.6-sol` 和 `high`。控制端会周期核对数据库期望状态、远端活动配置哈希、资源指纹、sing-box 状态和公网连通性；连续异常会自动生成一致性失败任务并唤醒 Codex 诊断。诊断不会未经确认重放高风险网络操作。
6. 在 `单节点库` 或 `订阅库` 导入资源。
...
应用新节点时目标先保存备份和服务状态，再启动自动回滚 timer。原 SSH 命令返回后，目标通过公开的 systemd activation timer 延迟重启 TUN，确保已运行的 sing-box 也加载新文件。Agent 只在进程稳定后提升待加载配置 SHA-256 和资源指纹；控制端用全新 SSH 连接核对活动标识、服务和公网出口。任一检查失败时不更新数据库当前节点，rollback timer 会恢复切换前状态。

失败任务的 Codex 结论只代表完成分析。相同参数后续复测成功时，任务中心才会标记 `已由任务 #X 验证解决`，并链接到成功任务。
