...
测速显示状态和真实访问延迟；详情分开展示 TCP、代理握手、真实 HTTPS 延迟和测试代理返回的出口 IP。
...
测速任务完成后，状态消息和任务详情提供 `查看各节点测速结果`。节点列表显示可用状态与真实访问延迟；点击节点可查看 DNS、TCP、代理握手、真实访问延迟及失败原因。

选择节点/订阅条目后，点击 `设为当前出口` 并二次确认。一个 VPS 任一时刻只记录一个当前代理出站。

切换任务完成后会显示所选节点和系统实际出口，并提供 `查看此 VPS`。VPS 详情中的“配置一致性”只有在数据库选择、活动配置 SHA-256、资源指纹和运行服务均匹配时才显示“已核对”。重复选择已经核对的当前节点不会再次执行网络修改。
...
普通任务显示任务号、类型、状态和进度。测速任务支持请求取消；取消在当前远端子测试结束后生效。Codex 任务单独标记，展示候选初始化或自动诊断进度。

普通任务发生系统级失败时，会自动排队 Codex 诊断，无需管理员另外发消息。Worker 主动通知“开始诊断”和最终结论；任务详情同时提供 `查看 Codex 自动诊断`。节点不可用仍作为正常测速结果保存，不会为每个离线节点启动 Codex。

后台一致性巡检默认每 60 秒运行，自动避开正在切换的 VPS。连续两次发现控制端与目标机不同步时，会生成 `consistency_check` 失败任务并唤醒 Codex 诊断；30 分钟内同一 VPS 不重复告警。为避免无确认重放高风险操作，Codex 自动介入默认负责分析和给出恢复入口，不自行切换节点。

诊断表示“已经分析”，不等于“已经修复”。同一 VPS、同一任务类型和相同参数后续执行成功时，旧失败任务会显示 `已由任务 #X 验证解决`，并提供按钮打开成功任务；没有该标记时应视为尚未经过复测确认。
